...
const unsigned short led[] = { 3, 2, 2, 3 };  // OVCD v1
...
const float mxLt = 2.6953, miLt = 1.5918, ctmxLt = 3.914, ctNorLt = 3.75;  // max values for 270 and 170 volt

double nu[] = { 100.522, 3.535 };
//function
void measuremaxmin();
...
void loop() {
  ...
  } else if (((tp == 'N') || (tp == '1') || (HighPtFlag == 1)) && (peak[0] < miLt))  // UV
  {
    ...
    countTime = millis();
    whileCon('U', 200, 1);
  } else if (((tp == 'Y') || (tp == '1')) && (mx[0] < (mxLt - 0.04)) && (peak[0] > (miLt + 0.04)) && (peak[1] < (ctNorLt) && (Irms < 5.5)) && (OCcount < 4))  // NV
  {
    tp = 'Y';
  ...
  while (whileExc) {
    ...
    switch (op) {
      ...
      case 'U':
        ...
        break;
      case 'N':
        condi = (mx[0] < (mxLt - 0.04)) && (peak[0] > (miLt + 0.04) && ((peak[1] < ctNorLt)) && (Irms < 5.5));
...
void whileCurrent() {
  ...
  if (outputValue[1] > ctmxLt)  // Oc
  {
    ...
    unsigned short currentripcount = 9;
    unsigned long octime = millis();
    unsigned short octimeout = 20;
