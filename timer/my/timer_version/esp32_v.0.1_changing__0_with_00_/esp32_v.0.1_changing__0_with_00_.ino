...
int ir = 0;
char SW, manualStatus = '0';
//*******************rt*******************
char daysOfTheWeek[7][12] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };
struct realTimeClock {
  unsigned short Temp, Hr, Min, Sec = 0, Date, Mon, Year;
  String Day;
} rt;
//*******************BT*******************
//c015424052022tue/t009301130030/m0
struct Timer {
  unsigned short onTimeMin, onTimeHr, offTimeMin, offTimeHr, dayOption, en;
  char st = 'f';
} timer[2];

//*******************function*******************
String op(int i);
char* onesToTens(unsigned short* num);  // add 0 in lcd if num is less than 9
...
void loop() {
  ...
  if ((millis() - present) > 1000) {
    Serial.println(String(present) + "___" + String(ir));
    present = millis();
    ir++;
  }
  if (ir > 50) {
...
void btRead() {
  ...
  if (serialData[1] == 'n') {
  ...
  } else if ((serialData[1] == 'c') && (serialData.length() == 17))
   {
    ...
    String value = String(serialData[2]) + String(serialData[3]);
    rt.Hr = value.toInt();
    value = String(serialData[4]) + String(serialData[5]);
    rt.Min = value.toInt();
    value = String(serialData[6]) + String(serialData[7]);
    rt.Date = value.toInt();
    value = String(serialData[8]) + String(serialData[9]);
    rt.Mon = value.toInt();
    value = String(serialData[10]) + String(serialData[11]) + String(serialData[12]) + String(serialData[13]);
    rt.Year = value.toInt();
    value = String(serialData[14]) + String(serialData[15]) + String(serialData[16]);
    rt.Day = value;
    ...
    if (timer[numTimer].offTimeHr != value) {
      ...
      EEPROM.write(add + 2, timer[numTimer].offTimeHr);
      EEPROM.commit();
    }
    ...
    if (timer[numTimer].offTimeMin != value) {
      ...
      EEPROM.write(add + 3, timer[numTimer].offTimeMin);
      EEPROM.commit();
    }
    ...
    if (timer[numTimer].dayOption != value) {
      ...
      EEPROM.write(add + 4, timer[numTimer].dayOption);
      EEPROM.commit();
    ...
    if (timer[numTimer].en != value) {
      ...
      EEPROM.write(add + 5, timer[numTimer].en);
      EEPROM.commit();
    }
...
void read_Rtc() {  //Read from RTC and store in rtcData;rtcTime,rtcDay
...
/*void onesToTens(unsigned short* num)
 { Serial.print(" in onesToTens ");
  if(*num < 10){lcd.print('0'); Serial.print('0');}
  lcd.print(*num);Serial.println(*num);
  }*/
char* onesToTens(unsigned short* num) {  // Serial.print(" in onesToTens ");
  char* temp;
  temp = (char*)malloc(3);
  if (*num < 10) {
...

{
  ...
  for (int i = 0; i <= 1; i++) {
    ...
    lcd.print(i);
    lcd.print(' ');
    lcd.print(onesToTens(&timer[i].onTimeHr));
    lcd.print(':');
    lcd.print(onesToTens(&timer[i].onTimeMin));
    lcd.print(' ');
    lcd.print(onesToTens(&timer[i].offTimeHr));
    lcd.print(':');
    lcd.print(onesToTens(&timer[i].offTimeMin));
    lcd.print(' ');
    lcd.setCursor(0, 1);
    lcd.print(" OP ");
    lcd.print(onesToTens(&timer[i].dayOption));
    lcd.print(" En ");
    lcd.print(timer[i].en);
    lcd.print(" St ");
    lcd.print(timer[i].st);
    delay(3000);
    if (i == 1) {
      break;  //getting error
    }
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(onesToTens(&rt.Hr));
  lcd.print(':');
  lcd.print(onesToTens(&rt.Min));
  lcd.print(' ');

  lcd.print(onesToTens(&rt.Date));
  lcd.print('/');
  lcd.print(onesToTens(&rt.Mon));
  lcd.print('/');
  lcd.print(rt.Year);
  lcd.print(' ');
  lcd.setCursor(0, 1);
...
void comp() {
  ...
  for (int index = 0; index < 2; index++) {
    ...
    int x = timer[index].dayOption;
    Serial.println(" timer " + String(index));
    String temp = op(x);
    delay(250);
    unsigned long currentTime = (rt.Hr * 100) + rt.Min;
    
    if (((SW == 'H') || (timer[index].st == 'f')) && (timer[index].en == 1) && ((temp.indexOf(rt.Day) >= 0) && (23 > temp.indexOf(rt.Day)))) {

      if (((currentTime) >= ((timer[index].onTimeHr * 100) + timer[index].onTimeMin)) && ((currentTime) < ((timer[index].offTimeHr * 100) + timer[index].offTimeMin))) {
        timer[index].st = 'o';
        digitalWrite(relaySw, LOW);
        SW = 'L';
      ...
      Serial.println("------IN off------");
      if (((timer[index].offTimeHr * 100) + timer[index].offTimeMin) <= (currentTime)) {
        timer[index].st = 'f';
        digitalWrite(relaySw, HIGH);
        SW = 'H';
        Serial.println("------Relay is off------");
