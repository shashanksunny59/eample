...
SerialBT.println("Recived TIMER VALUE");
  ...
  {
    if (serialData[2] == '0')
    { digitalWrite(TIMER_SW, HIGH); Serial.println(" Realy is OFF ");SerialBT.println("m-f");
    }
    else if (serialData[2] == '1')
    { digitalWrite(TIMER_SW, LOW); Serial.println(" Realy is ON ");SerialBT.println("m-o");
    }
...
SerialBT.print( String(rt.Date) + String(",") + String(rt.Mon) + String(",") + String(rt.Year)+","+String(rt.Hr) + String(",") + String(rt.Min)+","+String(rt.Temp));
delay(1000);
...
{ Serial.println("***********IN Comp***********");
  ...
  int x =timer[index].dayOption;Serial.println(" timer " + String(index));
    ...
    if ((timer[index].st == 'f') && (timer[index].en == 1) && (( temp.indexOf(rt.Day) >= 0) && (23 > temp.indexOf(rt.Day))))
    {
      ...
      if ((((rt.Hr * 100) + rt.Min) >= ((timer[index].onTimeHr * 100) + timer[index].onTimeMin)) && (((rt.Hr * 100) + rt.Min) < ((timer[index].offTimeHr * 100) + timer[index].offTimeMin)))
      {
        ...
        Serial.println("------Relay is on------");SerialBT.println(String(index)+"/"+"o");
      }
    }
    else if ((timer[index].st == 'o') && (timer[index].en == 1) && (( temp.indexOf(rt.Day) >= 0) && (23 > temp.indexOf(rt.Day))))
    {Serial.println("------IN off------");
      if (((timer[index].offTimeHr * 100) + timer[index].offTimeMin) <= ((rt.Hr * 100) + rt.Min))
      {
        timer[index].st = 'f';
        digitalWrite(TIMER_SW, HIGH);
        Serial.println("------Relay is off------"); SerialBT.println(String(index)+"/"+"f");
      }
    }
